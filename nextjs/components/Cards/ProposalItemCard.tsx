...
export default function ProposalItemCard({
  proposal,
  children,
}: {
  proposal: Proposal;
  ...
  const editor = useEditor({
    ...
    content: JSON.parse(proposal.cover_letter || "{}"),
  });
  ...
  return (
    <Box sx={{ minHeight: 150 }}>
      ...
      >
        <Typography
          level="h1"
          sx={{ fontSize: "md", fontWeight: "bold", color: "#495057" }}
          mb={0.5}
        >
          <Stack
            direction="row"
          ...
          >
            ...
            <CustomLink href={`freelancers/${proposal.freelancer_id}`}>
              <Chip
                color="neutral"
                variant="outlined"
                startDecorator={
        ...
        <Typography
          level="h6"
          sx={{ fontSize: "sm", fontWeight: "light", color: "#495057" }}
          mb={0.5}
        >
          Est. Budget {proposal.price.toFixed(2)} $ | Submitted{" "}
        ...
        <Stack direction="row" alignItems="center" spacing={1}>
          {proposal.status === Proposal_Status.Approved && (
            <Chip size="sm" color="success">
              {proposal.status.split("_").join(" ").toLowerCase()}
            </Chip>
          )}
